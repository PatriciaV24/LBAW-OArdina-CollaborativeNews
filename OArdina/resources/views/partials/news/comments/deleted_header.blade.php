...
<p class="text-white px-2 m-0">
    <small>
        <a class="col-auto pe-2" href="../user/{{ $comment->content->author->username  }}">
            Utilizador Apagado
        </a>
    ...
    @if (Auth::user() && (Auth::user()->is_admin || Auth::user()->id === $comment->content->author_id))
        <button class="clickable-big   ps-2" 
    ...
    @else
        ...
        <button class="clickable-big   ps-2 text-white" 
                data-bs-toggle="modal" 
